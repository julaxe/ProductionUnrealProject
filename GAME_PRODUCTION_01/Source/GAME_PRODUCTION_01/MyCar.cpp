...
void AMyCar::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	//Axis
	PlayerInputComponent->BindAxis(NAME_ThrottleInput, this, &AMyCar::ApplyThrottle);
	PlayerInputComponent->BindAxis(NAME_SteerInput, this, &AMyCar::ApplySteering);

	PlayerInputComponent->BindAxis("LookUp", this, &AMyCar::LookUp);
	PlayerInputComponent->BindAxis("LookRight", this, &AMyCar::LookRight);

	//Actions
	PlayerInputComponent->BindAction("HandBreak", IE_Pressed, this, &AMyCar::OnHandBrakePressed);
	PlayerInputComponent->BindAction("HandBreak", IE_Released, this, &AMyCar::OnHandBrakeReleased);

}

void AMyCar::ApplyThrottle(float val)
{
	GetVehicleMovementComponent()->SetThrottleInput(val);
}

void AMyCar::ApplySteering(float val)
{
	GetVehicleMovementComponent()->SetSteeringInput(val);
}

void AMyCar::LookUp(float val)
{
	if (val != 0.0f)
	{
...
void AMyCar::LookRight(float val)
{
	if (val != 0.0f)
	{
...
void AMyCar::UpdateInAirControl(float DeltaTime)
{
}